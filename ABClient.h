...
namespace asmjs
{
	...
	public:
		...
		void RpmRaw(uint64_t address, void *data, uint64_t size)
		{
			...
			req.Address = address;
			req.DataSize = size;
			WritePipe(req);  //Write the request
			ReadPipeRaw(buff, buffSize); //Read the responce
		...
		{
			T buffer;
			RpmRaw(address, &buffer, sizeof(T));
			return (buffer);
		}
		...
		std::string RPMString(SIZE_T address)
		{
			...
			RpmRaw(address, &name, sizeof(name));
